...
local load_scope_file = function(scope_file)
	...
	state.scopes[scope_name] = {}
	local patterns = state.scopes[scope_name]
...
local function reset()
	state.actived_scope = nil
end

local function load_scope(scope)
	state.actived_scope = scope
end

local function pick_scope()
	local scopes = vim.tbl_keys(state.scopes)
	pickers.new({}, {
...
local function search_in_scope(scope)
	local patterns = state.scopes[scope]
...
local function search_in_current_scope()
	if state.actived_scope == nil then
		return
	end
	search_in_scope(state.actived_scope)
end
...
M = {
	...
	search_in_current_scope = search_in_current_scope,
	state = state
}
